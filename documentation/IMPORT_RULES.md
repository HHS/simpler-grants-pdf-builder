...
- The `about:blank` placeholder (what Word and Google Docs write when a hyperlink in the source document has no destination) is also reported by `find_external_links`, flagged `invalid_destination` so it is shown as "no destination / not checked" and **never requested over HTTP**. It is a defect in the source `.docx`, not a broken anchor into the NOFO. Anchors with `about:blank` and no visible link text are skipped entirely, since there is nothing for a designer to find and fix. Note that this is a *classification* change only: nothing about the `.docx` import pipeline rewrites or removes these links, which is why there is no `IMPORT-NNN` entry for it.
