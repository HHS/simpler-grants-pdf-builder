...
class NofoImportJsonViewTest(TestCase):
    ...
    def test_regular_user_cannot_get_import_page(self):
        # Log in as regular user
        self.client.login(username="regular@groundhog-day.com", password="password")
    ...
    def test_regular_user_cannot_import_valid_nofo_json_file(self):
        # Log in as regular user
        self.client.login(username="regular@groundhog-day.com", password="password")

        _, uploaded_json_file = self._load_json_fixture("cms-u2u-25-001.json")
        ...
        # Assert that no NOFO with this number exists in the database
        self.assertFalse(
            Nofo.objects.filter(number="CMS-2U2-25-001").exists(),
            "A NOFO object was created when it shouldn't have been.",
        )
