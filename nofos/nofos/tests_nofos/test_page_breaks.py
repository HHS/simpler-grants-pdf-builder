...
class NofoRemovePagebreaksViewTest(TestCase):
    ...
    def test_get_context_data_counts_pagebreaks(self):
        ...
        self.assertEqual(
            response.context["pagebreak_count"], 8
        )  # 2 CSS + 6 word occurrences
    ...
    def test_post_removes_approved_pagebreaks(self):
        ...
        self.assertRedirects(
            response, reverse("nofos:nofo_edit", kwargs={"pk": self.nofo.pk})
        )
    ...
    def test_word_page_break_detection(self):
        ...
        self.assertEqual(
            response.context["subsection_matches"][0]["subsection"], subsection
        )
    ...
    def test_css_page_break_detection(self):
        ...
        highlighted1 = extract_page_break_context(
            subsection1.body, subsection1.html_class
        )
        self.assertIn("Page break at top of section found", highlighted1)

        # Test page-break-after
        highlighted2 = extract_page_break_context(
            subsection2.body, subsection2.html_class
        )
        self.assertIn("Page break at top of section found", highlighted2)

        # Test both
        highlighted3 = extract_page_break_context(
            subsection3.body, subsection3.html_class
        )
    ...
    def test_extract_page_break_context_method(self):
        ...
        self.assertIn(
            "<p><em>Page break found in CSS classes or other locations</em></p>", result
        )

        # Test with word page-break
        result = extract_page_break_context("Content with page-break word")
        self.assertIn(
            '<strong><mark class="bg-yellow">page-break</mark></strong>', result
        )
    ...
    def test_edge_cases(self):
        ...
        for subsection in [
            ...
            if subsection == empty_subsection:
                ...
                self.assertEqual(
                    count_page_breaks_subsection(subsection), 2
                )  # 2 CSS classes
            elif subsection == content_only_subsection:
                self.assertEqual(
                    count_page_breaks_subsection(subsection), 1
                )  # 1 word occurrence
