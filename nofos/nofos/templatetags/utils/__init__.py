...
# Word document bookmarks (internal links) are truncated to 40 characters
DOCX_MAX_BOOKMARK_LEN = 40
# Pattern: "<digits>--..."
DOCX_SAFE_ID_PATTERN = re.compile(r"^\d+--")  # id form, no leading '#'
...
def truncate_heading_ids(value):
    """
    Truncate a *bookmark/id name* to Word's max bookmark length,
    but only if it matches "<digits>--...".
    """
    if not value:
        return value
    ...
    # If it doesn't match the "digit--" convention, leave it alone
    if not DOCX_SAFE_ID_PATTERN.match(value):
        return value

    if len(value) <= DOCX_MAX_BOOKMARK_LEN:
        return value

    return value[:DOCX_MAX_BOOKMARK_LEN]


def truncate_anchor_links(html_string):
    """
    Truncate internal anchor href fragments to max_len characters (Word bookmark-safe),
    but ONLY for anchors that match "#<digits>--...".

    Example:
      #7--step-1-review-the-opportunity--eligibility
    becomes:
      #7--step-1-review-the-opportunity--eligib   (40 chars after '#')
    """
    if not html_string:
        return html_string
    ...
    for a in soup.find_all("a"):
        ...
        # Only internal fragment links
        if not href.startswith("#"):
            continue

        raw = href[1:]  # strip '#'
        a["href"] = "#" + truncate_heading_ids(raw)
