...
# Pattern: "#<digits>--..."
# this is a regex for identifying relative links generated by the Builder
DOCX_SAFE_ANCHOR_PATTERN = re.compile(r"^#\d+--")
...
def truncate_anchor_links(html_string, max_len=40):
    """
    ...
    for a in soup.find_all("a"):
        ...
        # Only touch internal anchors that start "#<digits>--"
        if not DOCX_SAFE_ANCHOR_PATTERN.match(href):
            continue

        fragment = href[1:]  # drop '#'
        if len(fragment) <= max_len:
            continue

        a["href"] = "#" + fragment[:max_len]
